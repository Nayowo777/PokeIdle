...
- **Android 后台挂机** — 开启设置中的后台挂机后，锁屏或切到后台仍会继续普通道路行走、导航、孵蛋、获得道具并自动遇敌抓捕，并通过常驻通知显示运行状态；后台运行会增加耗电；
...
当前 Android 增强分支地址：<https://github.com/Nayowo777/PokeIdle/tree/feature/save-transfer>。该分支的正式 APK 文件名以构建版本为准，生成后可直接安装到 Android 8.0（API 26）及以上设备。后台挂机会按普通道路规则继续累计行走距离、推进导航、计算孵蛋进度，并获得糖果、精灵球等道路道具，同时复用有效随从的相关加成。
